...
-- ผลวิเคราะห์ล่าสุดของแต่ละกลุ่ม (อัปเดตทุกรอบเก็บรายชั่วโมง) — ต้องการอะไร · หัวข้อ · นัด Meet
create table if not exists group_insights (
  source_id text primary key,
  data jsonb not null,                  -- {status, needs[], topics[], meetings[]}
  msgs int,                             -- วิเคราะห์จากกี่ข้อความ
  updated_at timestamptz default now()
);
...
-- push LINE ครั้งล่าสุด (แยกจาก last_report_at ที่ขยับทุกรอบสรุปรายชั่วโมง)
-- default now() = กลุ่มเดิมเริ่มนับจากตอนอัปเกรด ไม่ยิงสรุปเก่าที่เคยส่งไปแล้วซ้ำ
alter table watched add column if not exists last_push_at timestamptz default now();
`;
