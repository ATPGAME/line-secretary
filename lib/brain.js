...
import { q } from './db.js';
import { PERSONA, ORG } from './persona.js';
...
export async function salesReport(userId, args = {}) {
  ...
  if (!cur.length)
    return `ยังไม่มียอดขายช่วง ${short(start)}–${short(end)} — ส่งยอดมาได้เลย เช่น "ยอดวันนี้ Beta Oil 12,000"`;
...
// ปฏิทิน 7 วันข้างหน้าแบบ ค.ศ. พร้อมชื่อวัน — โมเดลบางตัวนับ "ศุกร์นี้" เองแล้วพลาด หรือใส่ปี พ.ศ. ลงช่องวันที่
const nowTime = () => new Date().toLocaleTimeString('en-GB', { timeZone: 'Asia/Bangkok', hour: '2-digit', minute: '2-digit' });
export const thaiWeekday = (d) => new Date(`${d}T12:00:00Z`).toLocaleDateString('th-TH', { weekday: 'long', timeZone: 'UTC' });
function weekCalendar(days = 8) {
  return Array.from({ length: days }, (_, i) => {
    const d = addDays(bkkDate(), i);
    return `${d} ${thaiWeekday(d)}${i === 0 ? ' (วันนี้)' : i === 1 ? ' (พรุ่งนี้)' : ''}`;
  }).join('\n');
}

// ── วิเคราะห์กลุ่ม: ตอนนี้กลุ่มต้องการอะไร · หัวข้อที่คุย · นัด Meet ที่ควรมี → JSON ลงกระดาน
// ส่งผลรอบก่อนเข้าไปด้วย ให้ AI "อัปเดตต่อ" — เรื่องที่ยังค้างอยู่ต่อ เรื่องที่จบแล้วตัดออก (ไม่ใช่เริ่มนับใหม่ทุกชั่วโมง)
const PRIORITY = ['สูง', 'กลาง', 'ต่ำ'];
const SLOTS = ['เช้า', 'บ่าย', 'เย็น'];
export async function analyzeGroup(title, rows, prev) {
  const lines = rows.map((r) => {
    const at = new Date(r.ts).toLocaleString('th-TH', {
      timeZone: 'Asia/Bangkok', day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit',
    });
    return `[${at}] ${r.who ? `${r.who}: ` : ''}${r.text}`;
  });
    ...
    [
      {
        role: 'system',
        content: [
          'คุณคือเลขาที่อ่านกลุ่ม LINE แทน Manager แล้วสรุปว่า "ตอนนี้กลุ่มนี้ต้องการอะไรจากเขา" ตอบเป็น JSON เท่านั้น ไม่มี markdown',
          'ใครเป็นใคร:', ORG,
          `ตอนนี้ ${nowTime()} น. · เวลางาน จันทร์-ศุกร์ 09:00-18:00 · ปฏิทิน (ค.ศ.):`, weekCalendar(),
          'รูปแบบ:',
          '{"status":"สภาพกลุ่มตอนนี้ 1 ประโยค","needs":[{"what":"สิ่งที่ต้องทำ/ตอบ/ตัดสินใจ เขียนให้ลงมือได้","from":"ใครขอ","owner":"ใครต้องทำ (คุณเกม/ชื่อคน/ทีม)","priority":"สูง|กลาง|ต่ำ","why":"ทำไมสำคัญ สั้น ๆ","plan_date":"YYYY-MM-DD","plan_slot":"เช้า|บ่าย|เย็น","state":"รอทำ|กำลังทำ|รอคำตอบ"}],' +
            '"topics":[{"title":"หัวข้อที่คุยกัน","detail":"ใจความ 1 ประโยค","who":["ชื่อ"]}],' +
            '"meetings":[{"title":"หัวข้อประชุม","why":"ทำไมควรคุยกันสด","with":["ชื่อ"],"date":"YYYY-MM-DD","time":"HH:MM","minutes":30,"confirmed":false}]}',
          'กฎ:',
          '- เอาเฉพาะที่มีในแชทจริง ห้ามแต่งเพิ่ม · คุยเล่น/ทดสอบระบบ ไม่นับเป็นงาน',
          '- เรื่องที่ N (ผู้บริหาร) ขอหรือถาม = priority สูง และ plan_date ไม่เกินวันทำการถัดไป',
          '- คำถามที่ยังไม่มีใครตอบ = need ที่ owner คือคุณเกม (หรือคนที่ถูกถาม) state "รอคำตอบ"',
          '- plan_date/plan_slot = วันที่ควรลงมือ เลือกจากปฏิทินข้างบน วันทำงานเท่านั้น สูง=วันนี้/พรุ่งนี้ กลาง=ภายใน 3 วัน ต่ำ=ภายในสัปดาห์',
          '- meetings: มีคนนัดประชุม/โทร/Meet ในแชทจริง → confirmed=true ใช้วันเวลาตามแชท',
          '- meetings ที่เลขาเสนอเอง (confirmed=false): เสนอ 1 นัดเมื่อ (ก) N สั่ง/ติดตามงานสำคัญตั้งแต่ 2 เรื่องขึ้นไปในกลุ่มนี้ → "อัปเดตงานกับ N" 15-30 นาที วันทำการถัดไป' +
            ' หรือ (ข) เรื่องต้องตัดสินใจร่วมกันหลายคน/คุยในแชทวนไม่จบ/ข้อมูลไม่ครบต้องคุยสด → เลือกช่วง 10:00-11:30 หรือ 14:00-16:00 วันทำงาน · ไม่เข้าเงื่อนไขให้เป็น []',
          '- ผลรอบก่อน: เรื่องที่ในแชทบอกว่าเสร็จ/ตอบแล้ว ให้ตัดออก · เรื่องที่ยังค้างให้เก็บไว้ (ปรับวันถ้าเลยมาแล้ว)',
          '- topics ไม่เกิน 5 · needs ไม่เกิน 8 · ใช้ชื่อคนตามที่เห็นในแชท',
        ].join('\n'),
      },
      {
        role: 'user',
        content: `กลุ่ม "${title || 'ไม่มีชื่อ'}"\n\nผลรอบก่อน:\n${JSON.stringify(prev || {})}\n\nข้อความ ${rows.length} ข้อความล่าสุด:\n${lines.join('\n') || '(ไม่มี)'}`,
      },
    ],
    false
  );
  const raw = JSON.parse(plain(msg.content).replace(/```json?|```/g, '').trim());
  // กันโมเดลใส่ค่ามั่ว: วันที่ต้องเป็น ค.ศ. · priority/slot ต้องอยู่ในรายการ
  const date = (d) => {
    const ce = toCE(d);
    return isDate(ce) ? ce : bkkDate();
  };
  const arr = (a) => (Array.isArray(a) ? a : []);
  return {
    status: String(raw.status || '').slice(0, 300),
    needs: arr(raw.needs).slice(0, 8).map((n) => ({
      what: String(n.what || '').slice(0, 300),
      from: n.from || '', owner: n.owner || '', why: n.why || '',
      priority: PRIORITY.includes(n.priority) ? n.priority : 'กลาง',
      plan_date: date(n.plan_date),
      plan_slot: SLOTS.includes(n.plan_slot) ? n.plan_slot : 'บ่าย',
      state: n.state || 'รอทำ',
    })).filter((n) => n.what),
    topics: arr(raw.topics).slice(0, 5).map((t) => ({ title: t.title || '', detail: t.detail || '', who: arr(t.who) })).filter((t) => t.title),
    meetings: arr(raw.meetings).slice(0, 4).map((m) => ({
      title: m.title || '', why: m.why || '', with: arr(m.with),
      date: date(m.date), time: /^\d{2}:\d{2}$/.test(m.time || '') ? m.time : '10:00',
      minutes: Math.min(180, Math.max(15, Number(m.minutes) || 30)), confirmed: !!m.confirmed,
    })).filter((m) => m.title),
  };
}
...
function systemPrompt(state, ctx) {
  ...
  return [
    ...
    `ตอนนี้ ${nowTime()} น. เวลาไทย · ปฏิทิน (ค.ศ.):\n${weekCalendar()}`,
