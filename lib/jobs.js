...
import { summarize, extractOrders, dueTime, salesReport, bkkDate, addDays, analyzeGroup } from './brain.js';
...
// collect = เก็บ+สรุปทุกกลุ่มขึ้น dashboard ทุกรอบ (รายชั่วโมง) แต่ push LINE เฉพาะชั่วโมงใน report_hours
// สองขั้นแยกกัน: สรุปข้อความใหม่ → ตาราง reports · push = รวม reports ตั้งแต่ push ครั้งก่อนเป็นข้อความเดียว
export async function report({ force = false, collect = false } = {}) {
  // ไม่บังคับว่าต้องมี report_to — ยังไม่ตั้ง OWNER_USER_ID ก็ยังอยากให้สรุปขึ้น dashboard
  const { rows: groups } = force || collect
  ...
  const one = async (g) => {
    try {
      ...
      // ไม่มีอะไรใหม่ก็ไม่ต้องกวน (และไม่เปลืองโควตา push) — แต่กลุ่มที่ยังไม่เคยวิเคราะห์ให้ทำรอบแรก
      if (rows.length === 0) {
        if (collect || force) await refreshInsight(g, g.title, { onlyIfMissing: true });
      ...
      if (g.track_orders) await saveOrders(g.source_id, rows);

      // ลง dashboard — push ทำทีหลังจากตาราง reports (ถ้า push พลาด สรุปไม่หาย)
  ...
  const pushed = await pushDigest(groups, { force });
  return { sent, failed, ...(pushed && { pushed }) };
}

// รวมสรุปตั้งแต่ push ครั้งก่อนของแต่ละกลุ่ม → ข้อความเดียวต่อคนรับ
// (ยิงแยกทีละกลุ่มกินโควตาฟรี 300/เดือน หมดตั้งแต่ 5 กลุ่ม · ยิงทุกชั่วโมงก็หมดใน 2 สัปดาห์)
async function pushDigest(groups, { force }) {
  const hour = hourBangkok();
  const due = groups.filter(
    (g) =>
      g.report_to &&
      (force || g.report_hours.includes(hour)) &&
      (!g.last_push_at || Date.now() - new Date(g.last_push_at) > 2 * 3600e3) // กัน cron สองตัวยิงซ้ำ
  );
  ...
  for (const g of due) {
    const { rows: reps } = await q(
      `select summary as text, period_start as ts from reports
        where source_id = $1 and created_at > coalesce($2::timestamptz, '-infinity') order by created_at`,
      [g.source_id, g.last_push_at]
    );
    if (!reps.length) continue;
    const title = g.title || g.source_id.slice(0, 8);
    // หลายรอบ (รายชั่วโมง) → ให้ AI ย่อรวมเป็นก้อนเดียว · รอบเดียวใช้ตามเดิม
    const body = reps.length === 1 ? reps[0].text : await summarize(title, reps);
    const extra = g.track_orders ? (await ordersLine(g.source_id, g.last_push_at)) + (await moneyLine(g)) : '';
    const block = `📋 ${title} (${reps.length} รอบสรุป)\n${body}${extra}`;
    digest.set(g.report_to, [...(digest.get(g.report_to) || []), block]);
    done.push(g.source_id);
  }

  const plans = digest.size ? await agendaLine() : '';
  let pushed = 0;
  for (const [to, blocks] of digest) if (await push(to, digestText(blocks, dashUrl(), plans))) pushed++;
  if (pushed) await q('update watched set last_push_at = now() where source_id = any($1)', [done]);
  return pushed;
}

// อ่านข้อความ 3 วันล่าสุดของกลุ่ม + ผลรอบก่อน → ให้ AI อัปเดตว่าตอนนี้กลุ่มต้องการอะไร (ลงตาราง group_insights)
// พังก็แค่ log — สรุปรายชั่วโมงลงกระดานไปแล้ว ไม่ควรล้มตามเพราะขั้นนี้
async function refreshInsight(g, title, { onlyIfMissing = false } = {}) {
  try {
    const { rows: [prev] } = await q('select data from group_insights where source_id = $1', [g.source_id]);
    if (onlyIfMissing && prev) return;
    const { rows } = await q(
      `select * from (select user_id, text, ts from messages
          where source_id = $1 and text is not null and ts > now() - interval '3 days'
          order by ts desc limit 300) m order by ts`,
      [g.source_id]
    );
    if (!rows.length && !prev) return;
    const data = await analyzeGroup(title, await withNames(g.source_id, rows), prev?.data);
    await q(
      `insert into group_insights (source_id, data, msgs, updated_at) values ($1,$2,$3,now())
       on conflict (source_id) do update set data = excluded.data, msgs = excluded.msgs, updated_at = now()`,
      [g.source_id, JSON.stringify(data), rows.length]
    );
  } catch (err) {
    console.error(`insight failed ${g.source_id}:`, err.message);
  }
}

async function ordersLine(sourceId, since) {
  const { rows: [o] } = await q(
    `select count(*)::int as n, coalesce(sum(amount), 0) as total from orders
      where source_id = $1 and ordered_at > coalesce($2::timestamptz, now() - interval '24 hours')`,
    [sourceId, since]
  );
  if (!o.n) return '';
  return `\n\n🧾 ออเดอร์ ${o.n} รายการ${Number(o.total) ? ` · รวม ${Number(o.total).toLocaleString('th-TH')} บาท` : ''}`;
}
...
export async function runJobs(opts = {}) {
  const { sent, failed, pushed } = await report(opts);
  ...
  return {
    reported: sent,
    ...(pushed && { pushed }), // ส่งสรุปเข้า LINE กี่คน (รอบเก็บรายชั่วโมงส่วนใหญ่ = 0)
