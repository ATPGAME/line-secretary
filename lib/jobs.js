...
import { summarize, extractOrders, dueTime } from './brain.js';
...
export async function report({ force = false } = {}) {
  ...
  for (const [to, blocks] of digest) await push(to, digestText(blocks));
  return sent;
}

// ต่อบล็อกสรุปเป็นข้อความเดียว ยาวเกินก็ตัด แล้วบอกให้ไปอ่านต่อบนกระดาน (ไม่ปล่อยให้ LINE ตัดเงียบ ๆ)
export function digestText(blocks, link = dashUrl()) {
  const head = `📊 สรุปรอบ ${thaiTime()} น. · ${blocks.length} กลุ่ม`;
  const tail = link ? `\n\nอ่านเต็ม ๆ ที่ ${link}` : '';
  const sep = '\n\n───────\n\n';
  const out = [];
...
export async function checkHealth({ notify = true } = {}) {
  ...
  if (days !== null && days <= 7)
    problems.push(['token', `LINE token เหลือ ${days} วันจะหมดอายุ — ออกใหม่ก่อนบอทเงียบ`]);
...
export async function runJobs(opts = {}) {
  ...
  return { reported, alerted, due, ...(health && { problems: health.problems }) };
}
